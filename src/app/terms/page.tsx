...
const DOCS: Doc[] = [
  ...
  },
  {
    ...
    subtitle: "Job updates, chat, and optional notifications.",
    requiredFor: ["all"],
    color: "#06B6D4",
    icon: "💬",
    body: "Messaging & SMS consent\n\n" + MESSAGING_CONSENT_BODY + SCROLL_FOOTER,
  },
  {
...
export default function TermsPage() {
  ...
  const handleEnterGRIDD = useCallback(async () => {
    setGateError(null);
    try {
      console.log("[terms] Enter GRIDD pressed");
      const auth = getAuth();
      const user = auth.currentUser;
      console.log("[terms] Current user:", user?.uid ?? null);

      if (!user) {
        console.warn("[terms] Enter GRIDD: no user signed in");
        setGateError("Please sign in first, then try again.");
      ...
      const userSnap = await getDoc(doc(db, "users", user.uid));
      const provSnap = await getDoc(doc(db, "providers", user.uid));
      console.log("[terms] users/{uid} exists:", userSnap.exists());
      console.log("[terms] providers/{uid} exists:", provSnap.exists());
      if (userSnap.exists()) console.log("[terms] users data role:", userSnap.data()?.role);
      if (provSnap.exists()) console.log("[terms] providers data keys:", Object.keys(provSnap.data() ?? {}));

      if (!userSnap.exists() && !provSnap.exists()) {
        console.warn("[terms] Enter GRIDD: no Firestore profile — redirect to role picker");
      ...
      if (userSnap.exists()) {
        ...
        if (!synced.ok) {
          console.warn("[terms] syncSession:", synced.error);
        }
        if (r === "ceo" || roleStr === "admin") {
        ...
        if (r === "driver") router.push("/driver/jobs");
      ...
      console.error("[terms] Enter GRIDD error:", err);
      setGateError(err instanceof Error ? err.message : "Something went wrong. Try again.");
    }
  }, [router]);
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-6 py-10">
        ...
        <div className="sticky bottom-0 z-30 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-2">
          <div className="rounded-2xl border border-[var(--border)] bg-[#060606]/95 p-4 backdrop-blur">
            {gateError ? <p className="mb-3 text-sm text-red-400">{gateError}</p> : null}
            <Button
              type="button"
              disabled={!allSigned}
              onClick={() => {
                if (!allSigned) return;
                void handleEnterGRIDD();
              }}
              className={[
